...
2. Select an **activity** or _day model_{:.label}.
...
- _Schedule_{:.menu-item}: displays manually or automatically scheduled activities and shifts. xref:scheduling:schedules-shift-bidding.adoc[Shift bidding] and xref:scheduling:schedules-optimized-schedules.adoc[Optimized schedules] write to this level. Job and break optimizations change the content, employees see it in Me.
- _Actual_{:.menu-item}: contains the actual schedule. xref:#tip-3-copy-the-original-schedule-as-reference[Copy your final schedule] to this level and make any last-minute changes for the day here.
- _External System_{:.menu-item}: displays xref:acd-integration:import-agent-status-data.adoc[agent status data] imported using an integration (if configured). You can add/edit entries manually if needed.
- _Request_{:.menu-item}: displays all normal shift requests that employees enter in Me. You can add day models manually, but no individual activities.
- _Alternative Request_{:.menu-item}: shows all alternative requests employees enter in Me. You can add day models manually, but no individual activities.
- _Vacation Request/Absence Request_{:.menu-item}: displays any time-off requests employees enter in Me. You can add day models manually, but no individual activities.
