= Importance value for activities
...
:related-articles: activity-types-and-properties.adoc

You can define an importance value for each activity in menu:Plan[Configuration>Activities].
...
Due to the five times lower importance value, the job optimization can assign five more employees to activity A before it has the same negative impact on the overall coverage as assigning one more employee to activity B. Thus, activity A tends to be overstaffed.