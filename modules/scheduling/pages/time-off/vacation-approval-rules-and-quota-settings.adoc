= Use approval rules and quota settings
...
:related-articles: vacation-entitlement.adoc, vacation-absences-management.adoc
...
New to Time Off? Learn the xref:scheduling:vacation-absences-management.adoc[basics] first.

[#configure-settings-for-suggestions]
== Configure settings for suggestions
1. Go to menu:Plan[Time Off]
2. Click btn:[Configure Suggestions].
...
- **The agent has enough entitlement left**: Approvals are possible if the agent has enough remaining xref:scheduling:vacation-entitlement.adoc[vacation entitlement] for the year.
- **The daily average staff requirements for the planning unit are covered (including sickness and other planned absences)**: Approvals are possible if enough people are working to cover the daily average xref:scheduling:edit-or-delete-staff-requirements.adoc[staff requirements] for all activities in the planning unit.
...
To save the configuration, click btn:[Save Rules].
...
6. Press the btn:[Save Quota] button.
...
[NOTE]
====
You can drag and drop the date ranges to rearrange them. To delete date ranges click the {icon-delete}.
====
[#weekdays]
=== Weekdays
...
Click btn:[Save Quota] when you have finished configuring the quota.
