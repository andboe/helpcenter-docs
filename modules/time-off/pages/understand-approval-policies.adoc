...
====
In menu:Time Off[], approval policies define how Peopleware handles time-off requests. Approval policies use xref:#what-are-checks[checks] and configurable thresholds to enforce company rules around time off.

You can configure specific approval policies for each link:https://help.peopleware.com/time-off-types-beta[time-off type]. When a person submits a time-off request, Peopleware evaluates it against the active approval policy for the selected type.
