...
[NOTE]
====
The URL of this API has been changed to *legacy-api.peopleware.com*. The old URL remains valid for now, giving you enough time to change your current API queries. The new Peopleware API documentation is available at link:https://api.peopleware.com[https://api.peopleware.com].
====
[#api-reference-available-endpoints]
== API reference: Available endpoints
The API reference link:https://legacy-api.peopleware.com[legacy-api.peopleware.com] describes all available endpoints and associated parameters. To learn more about the query parameters and return values, select an endpoint and click the green bar in the middle. To show the full URL, click the *text field showing the endpoint* in the upper right.
...
[NOTE]
====
To give this permission to other user roles, xref:account:configure-user-roles.adoc[edit the user role]. In the *Permissions* section, go to menu:Modules[My Profile>Personal access tokens] and check the checkbox.
====
Create your Personal Access Token in your *user profile*:

1. Click your *name on the start page*, to get into your *user profile*.
2. Select the *Personal Access Tokens* menu item
3. Click btn:[Generate new token]
4. Enter a description for your Personal Access Token
5. Click btn:[Generate new token]
...
The following screenshot shows the two headers in link:https://install.advancedrestclient.com/install[Advanced Rest Client] (ARC):
