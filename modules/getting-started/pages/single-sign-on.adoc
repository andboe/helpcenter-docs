...
## SSO für dein Konto aktivieren <a name="activate-sso-for-your-account"></a>
...
3.  Im Abschnitt **injixo SAML Metadaten** stehen dir zwei Optionen zur Verfügung, je nachdem, ob dein IdP Dateiuploads unterstützt:
    ...
    - Dateiuploads werden unterstützt: Klicke auf _{% icon download | icon-only %} Herunterladen _{:.doc-button} und vervollständige die IdP-Konfiguration mit der heruntergeladenen XML-Datei.
...
4.  (Optional) Wenn du verschlüsselte SAML-Assertions verwendest, klicke auf _{% icon download | icon-only %} Herunterladen<!-- DeepL übersetzt --> _{:.doc-button} im Abschnitt **Token-Verschlüsselung**. Füge die heruntergeladenen Zertifikate zu deiner IdP-Konfiguration hinzu. Das Zertifikat hat eine begrenzte Gültigkeit. Unsere Techniker ersetzen das Zertifikat sechs Wochen vor dem Ende der Gültigkeit durch ein neues. Beide Zertifikate werden während des sechswöchigen Zeitraums unterstützt.
...
6.  Klicke auf _Konfiguration speichern_{:.doc-button}.
    ...
    >
    > Um die Sicherheit noch weiter zu erhöhen, kannst du [SSO für alle Benutzer erzwingen](#enforce-sso-for-all-users).

## SSO-Konfiguration testen <a name="test-the-sso-configuration"></a>
...
### Test fehlgeschlagen? Konfiguriere die SAML-Antwort <a name="test-failed-configure-the-saml-response"></a>
...
<saml:Subject><saml:NameID Format="urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress">email@company.org</saml:NameID>
...
## SSO für alle Benutzer erzwingen <a name="enforce-sso-for-all-users"></a>
...
## E-Mail-Adressen ändern, wenn SSO erzwungen wird <a name="change-email-addresses-when-sso-is-enforced"></a>
...
## Über SSO anmelden <a name="log-in-using-sso"></a>
...
## Zugriff auf injixo entziehen <a name="revoke-access-to-injixo"></a>

Um den Zugriff eines Benutzers auf injixo über SSO zu entziehen, musst du im IdP die Benutzerzuordnung zu injixo löschen.

Beachte, dass der Entzug des Zugriffs des Benutzers in deinem IdP ihn nicht in injixo deaktiviert. Um den Zugriff eines Benutzers auf injixo vollständig zu entziehen, musst du {% link_new den Benutzer deaktivieren oder löschen | features/people/deactivate-people.md %}.

## SSO deaktivieren <a name="deactivate-sso"></a>
...
## SSO für mehrere Tenants verwenden <a name="use-sso-on-multiple-tenants"></a>
...
2.  Klicke den Mitarbeiter an, der Zugriff auf mehrere Tenants haben muss.
3.  Klicke im Konfigurationsfenster auf den Tab _Zugriffsrechte _{:.label}.
4.  Klicke im Abschnitt **Single Sign-On** auf {% icon pencil | icon-only %} **Bearbeiten**.
5.  Gib einen Benutzerprinzipalnamen.
6.  Klicke auf _Speichern_{:.doc-button}.

Die injixo E-Mail-Adresse des Benutzers, und nicht sein UPN, wird für alle E-Mail-Verifizierungsprozesse sowie für Benachrichtigungen oder Reports verwendet. Die E-Mail Adresse muss für jeden Tenant eindeutig sein. Du kannst [plus Adressierung](https://en.wikipedia.org/wiki/Email_address#Sub-addressing) verwenden.
