= Einen Forecast berechnen

Dieser Artikel enthält eine Übersicht der zentralen Schritte auf dem Weg zu deinem ersten \{% link_new Forecast | features/forecast/injixo-forecast/what-is-the-injixo-forecast.md %}. Auf Grundlage der historischen Daten berechnet der Forecast die Anzahl der Mitarbeiter, die für die Bearbeitung des eingehenden Volumens zu einer Aktivität in einer Planungseinheit erforderlich sind. Dieser Artikel enthält eine Übersicht über die Schritte zum Erstellen eines Forecasts. Um mehr zu jedem Schritt zu erfahren, folge den Links in diesem Artikel.
